-- Delivery acceptance window per store, used by the order-scheduling tool.
-- Left null until a user explicitly saves one — the app applies the M-F
-- 8:00 AM - 4:00 PM default in that case, so "unsaved" and "saved with the
-- default values" are distinguishable if that matters later.
alter table public.stores
  add column if not exists delivery_accepted_days smallint[],
